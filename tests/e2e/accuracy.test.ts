...
describe.skipIf(!HAS_FULL_STACK || !HAS_FIXTURES)('Accuracy: Chopin Op.28 No.1', () => {
  const outputPath = join(tmpdir(), `accuracy-chopin-${Date.now()}.musicxml`)
  const scoreMidiPath = outputPath.replace('.musicxml', '.mid')
  const perfMidiPath = outputPath.replace('.musicxml', '.perf.mid')
  ...
  it('runs the pipeline', async () => {
    await runPythonScript('pipeline', [FIXTURE_WAV, '-o', outputPath, '--solo-piano'])
  ...
  it('transcription F1 >= 0.4 at 100ms tolerance', () => {
    ...
    expect(result.f1).toBeGreaterThanOrEqual(0.4)
  })

  it('transcription F1 >= 0.3 at 50ms tolerance', () => {
  ...
  it.afterAll(() => {
    for (const f of [outputPath, scoreMidiPath, perfMidiPath]) {
